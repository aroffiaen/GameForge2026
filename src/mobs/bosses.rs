...
use crate::entities::ennemies::{spawn_enemy_projectile, EnemyKind};
use crate::mobs::spawn::spawn_enemy;
use crate::mobs::components::{Boss, BossAiTag};
use rand::prelude::*;
...
pub fn spawn_boss_specialized(
    ...
    crate::entities::ui::spawn_health_bar(commands, boss_id, 60.0, true, Some(kind.name()));
...
pub fn araignee_ai(
    ...
    player: Query<(Entity, &Transform, &Radius, &Iframes), With<Player>>,
    mut bosses: Query<(Entity, &mut Transform, &mut Velocity, &mut Araignee), (With<BossAiTag>, Without<Player>)>,
) {
            ...
            AraigneeState::Chase => {
                ...
                if spider.timer.is_finished() {
                    ...
                    if roll < 0.5 {
                        ...
                        // Télégraphe visuel (cercle au sol)
                        commands.spawn((
                            Sprite {
                                color: Color::srgba(0.9, 0.3, 0.1, 0.25),
            ...
            AraigneeState::Summon => {
                ...
                if spider.timer.is_finished() {
                    for _ in 0..rng.random_range(2..=3) {
                        ...
                        spawn_enemy(&mut commands, EnemyKind::Araignee, pos + off, 1.0, 1.0);
                    }
                    spider.state = AraigneeState::Chase;
...
pub fn scorpion_ai(
    ...
    mut bosses: Query<(&Transform, &mut Velocity, &mut Sprite, &BaseColor, &mut Scorpion), With<BossAiTag>>,
) {
...
pub fn gromp_ai(
    ...
    mut dmg: BevyMessageWriter<DamageMsg>,
    player: Query<(Entity, &Transform, &Radius, &Iframes), With<Player>>,
    mut bosses: Query<(Entity, &mut Transform, &mut Velocity, &mut Gromp), (With<BossAiTag>, Without<Player>)>,
) {
    let Ok((player_e, player_tf, player_r, iframes)) = player.single() else {
        return;
    };
            ...
            GrompState::Chase => {
                ...
                if gromp.timer.is_finished() {
                    if rng.random_bool(0.6) {
                        ...
                        commands.spawn((
                            Sprite {
                                color: Color::srgba(1.0, 0.4, 0.5, 0.3),
                                custom_size: Some(Vec2::new(270.0, 22.0)),
                                ..default()
                            },
                ...
                if gromp.timer.is_finished() {
                    ...
                    commands.spawn((
                        Sprite {
                            ...
                            custom_size: Some(Vec2::new(270.0, 14.0)),
                            ..default()
                        },
...
pub fn glob_system(
    ...
    for (e, tf, mut glob) in &mut globs {
        ...
        if glob.timer.is_finished() {
            ...
            commands.spawn((
                Sprite {
                    color: Color::srgba(0.6, 0.3, 0.7, 0.45),
